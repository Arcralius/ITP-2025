...
class TestFlaskRoutes:
    ...
    def test_download_success(self, mock_append_date, mock_verify):
    ...
    def test_download_no_json(self):
        ...
        assert response.json['error'] in ['Password is required', 'Invalid password']
    
    @patch('pdns_utils.KeySigServer.verify_password')
    @patch('pdns_utils.KeySigServer.append_date_to_filename')
    def test_download_file_not_found(self, mock_append_date, mock_verify):
        """Test download when zip file doesn't exist"""
        mock_verify.return_value = True
        mock_append_date.return_value = "nonexistent.zip"
